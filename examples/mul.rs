...
fn main() {
    ...
    a.truncate(n + m - 1);
    output.write(buf.format(a[0].get()).as_bytes()).unwrap();
    for a in a.into_iter().skip(1) {
        output.write(b" ").unwrap();
        output.write(buf.format(a.get()).as_bytes()).unwrap();
    }
    output.write(b"\n").unwrap();
}
